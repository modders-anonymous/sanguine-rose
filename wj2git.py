...
import wjdb
import momodlist
from momodlist import openModTxtFile
from momodlist import openModTxtFileW
...
def wj2git(config):
    ...
    allmods = {}
    mastermodlist = momodlist.ModList(mo2+'profiles\\'+masterprofilename+'\\')
    for mod in mastermodlist.allEnabled():
        allmods[mod]=1
    altmodlists = {}
    for profile in altprofilenames:
        modlist = momodlist.ModList(mo2+'profiles\\'+profile+'\\')
        for mod in modlist.allEnabled():
            allmods[mod]=1
    ...
    for profile in altmodlists:
        ...
        if filterout is None:
            ...
            for mod in modlist.modlist:
                separ = momodlist.ModList.isSeparator(mod)
                # print(separ)
                if separ:
                    # section = separ
